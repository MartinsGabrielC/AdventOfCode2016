data = '''vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]
...
def part1():
    ...
    for info in data:
        ...
        check = ""
        info = info.split(sep='-')
        rid = int(info[-1].split('[')[0])
        checksum = info[-1].split(sep='[')[-1].strip(']')
        del(info[-1])
        for code in info:
            for letter in code:
                if letter in letters:
                    ...
                    letters[letter]=1
        letters = OrderedDict(sorted(letters.items(), key=lambda x : x[0]))
        sortedLetters = sorted(letters.items(), key=lambda x : x[1],reverse=True)
        for i in range(5):
            check+=sortedLetters[i][0]
        if check == checksum:
            count+=rid
    print(count)

def shiftLetter(letter, shift):
    return strs[(strs.index(i) + shift) % 26]
    ...
    for info in data:
        ...
        for code in info:
            ...
            for indx, letter in enumerate(code):
                code[indx]= strs[(strs.index(letter) + rid) % 26]
            phrase+=''.join(code)+' '
        if "north" in phrase:
            print(rid)
            print(phrase)
